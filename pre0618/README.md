# pre0618 - Archive of earlier working files

**Author:** Yash Moitra
**Institution:** Delhi International Airport Limited
**Date:** July 05, 2026

Working files that predate the June 18, 2026 restructure, preserved for provenance.
Nothing here feeds the current model directly; the current pipeline starts from
`../asimplied.csv` (see the root `README.md` and `DATA_DICTIONARY.md`).

Contents: the original `Foundation Schedule.xlsx` and its rebuilt variant,
`Pax Arrival Times.xlsx` (the earlier arrival-time working), the `T3 Checkin Model/`
prototype (R Markdown), early build scripts (`build2.py`, `build_asreceived.py`,
`buildfull.py`, `asimplied_profile.R/.Rmd`), interim data (`df1.csv`, `_data.json`),
and photograph archives (`Photos-3-001.zip`).
